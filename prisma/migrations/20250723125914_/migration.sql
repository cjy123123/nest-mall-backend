...
CREATE TABLE `CartItem` (
    ...
    `goodsSpecId` INTEGER NOT NULL,
    `quantity` INTEGER NOT NULL DEFAULT 1,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,

    UNIQUE INDEX `CartItem_cartId_goodsSpecId_key`(`cartId`, `goodsSpecId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `GoodsSpec` ADD CONSTRAINT `GoodsSpec_goodsId_fkey` FOREIGN KEY (`goodsId`) REFERENCES `Goods`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `CartItem` ADD CONSTRAINT `CartItem_goodsSpecId_fkey` FOREIGN KEY (`goodsSpecId`) REFERENCES `GoodsSpec`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
